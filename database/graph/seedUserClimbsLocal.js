    ...
    db.query(`SELECT date, usa_boulders FROM ascents WHERE user_id = ${uniqueUsers[i].user_id};`, (err, results) => {
      ...
      if (userClimbs.length !== 0) {
        ...
        if (Object.keys(user).length !== 1 && Object.keys(user).length !== 0) {
          ...
          db.query(`INSERT INTO climbers (id,v0m,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9,v10) VALUES (${uniqueUsers[i].user_id}, ${user['V0-'] || null}, ${user.V0 || null}, ${user.V1 || null}, ${user.V2 || null}, ${user.V3 || null}, ${user.V4 || null}, ${user.V5 || null}, ${user.V6 || null}, ${user.V7 || null}, ${user.V8 || null}, ${user.V9 || null}, ${user.V10 || null})`, (err, results) => {
            if (err) throw err;
              if ((i + 1) % 500 === 0 || i + 1 === uniqueUsers.length) {
                console.log(`Successfully Seeded: ${i + 1}/${uniqueUsers.length}`);
              }
          })
        } else {
